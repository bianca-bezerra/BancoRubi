...
CREATE TRIGGER VALIDATE_EXISTENCIA_CONTA_SQ BEFORE INSERT ON SAQUEDEPOSITO FOR EACH ROW
EXECUTE PROCEDURE VERIFICAR_EXISTENCIA_CONTA('SAQUEDEPOSITO')

CREATE TRIGGER VALIDATE_EXISTENCIA_CONTA_TRANS BEFORE INSERT ON TRANSFERENCIA FOR EACH ROW
...
BEGIN
       -- Verifica se a operação é um saque
    IF TG_ARGV[0] = 'SAQUEDEPOSITO' THEN
		IF NEW.TIPO = 'S' THEN
        	IF NOT(SELECT SALDO FROM CONTA WHERE ID_CONTA = NEW.ID_CONTA) >= NEW.VALOR THEN
            	RAISE EXCEPTION 'SALDO INSUFICIENTE PARA SAQUE!';
        	END IF;
		END IF;
    -- Verifica se a operação é uma transferência
